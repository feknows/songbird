# SongBird — AGENTS.md

## Project

Sistema de registro de vendas (sales registration system). Portuguese-only (UI, code, comments).

Single-page app (`public/index.html`) no build step, talking to Supabase. Hosted on GitHub Pages. CSS and JS separated into `public/css/` and `public/js/` directories.

**Leads scraper** was separated into its own project at `C:\Users\Felipe\SongBird-Leads` — see its own `AGENTS.md`.

## Skills

- Sempre informar ao usuário quais skills serão utilizadas antes de iniciar qualquer tarefa.

## Auto-review

After every implementation (commit or group of related changes), the agent must perform a self-review evaluating whether the approach was optimal. If a better alternative exists, present it to the user with trade-offs so they can decide whether to keep, adjust, or revert. This applies to all changes regardless of perceived size.

After every implementation, also ensure `git add` → `git commit` → `git push` was executed.

## Supabase setup

1. Create a Supabase project.
2. Copy `.env.example` to `.env` locally (for `migrate-supabase.js`).
3. **Critical**: In `public/js/supabase-init.js:3-4`, replace the placeholder credentials with real Supabase URL and anon key.
4. Run the SQL below in the SQL Editor (creates schema + RLS policies):
   ```sql
   -- SongBird - Supabase Schema
   CREATE TABLE IF NOT EXISTS usuarios (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     nome TEXT NOT NULL UNIQUE,
     permissao TEXT NOT NULL DEFAULT 'user' CHECK (permissao IN ('admin', 'user')),
     auth_id UUID UNIQUE REFERENCES auth.users(id) ON DELETE SET NULL,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS produtos (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     nome TEXT NOT NULL,
     tipo TEXT NOT NULL,
     ativo INTEGER DEFAULT 1 CHECK (ativo IN (0, 1)),
     ordem INTEGER DEFAULT 0
   );
   CREATE TABLE IF NOT EXISTS faixas (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     produto_id BIGINT NOT NULL REFERENCES produtos(id) ON DELETE CASCADE,
     descricao TEXT NOT NULL,
     valor_base NUMERIC(10,2) NOT NULL DEFAULT 0,
     valor_modulo NUMERIC(10,2) NOT NULL DEFAULT 0,
     ordem INTEGER DEFAULT 0
   );
   CREATE INDEX IF NOT EXISTS idx_faixas_produto_id ON faixas(produto_id);
   CREATE TABLE IF NOT EXISTS modulos (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     produto_id BIGINT NOT NULL REFERENCES produtos(id) ON DELETE CASCADE,
     nome TEXT NOT NULL,
     ativo INTEGER DEFAULT 1 CHECK (ativo IN (0, 1)),
     ordem INTEGER DEFAULT 0
   );
    CREATE INDEX IF NOT EXISTS idx_modulos_produto_id ON modulos(produto_id);
   CREATE TABLE IF NOT EXISTS modulo_precos (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     modulo_id BIGINT NOT NULL REFERENCES modulos(id) ON DELETE CASCADE,
     faixa_id BIGINT NOT NULL REFERENCES faixas(id) ON DELETE CASCADE,
     valor NUMERIC(10,2) NOT NULL DEFAULT 0,
     UNIQUE(modulo_id, faixa_id)
   );
   CREATE INDEX IF NOT EXISTS idx_modulo_precos_modulo_id ON modulo_precos(modulo_id);
   CREATE INDEX IF NOT EXISTS idx_modulo_precos_faixa_id ON modulo_precos(faixa_id);
   CREATE TABLE IF NOT EXISTS leads (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     nome TEXT NOT NULL,
     telefone TEXT DEFAULT '',
     whatsapp TEXT DEFAULT '',
     site TEXT DEFAULT '',
     endereco TEXT DEFAULT '',
     avaliacao NUMERIC(3,1),
     redes_sociais TEXT DEFAULT '',
     cidade TEXT NOT NULL,
     termo TEXT NOT NULL,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS equip_categorias (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     nome TEXT NOT NULL,
     ordem INTEGER DEFAULT 0
   );
   CREATE TABLE IF NOT EXISTS equipamentos (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     categoria_id BIGINT NOT NULL REFERENCES equip_categorias(id) ON DELETE CASCADE,
     nome TEXT NOT NULL,
     status TEXT DEFAULT 'integrado',
     versao TEXT DEFAULT '',
     observacoes TEXT DEFAULT ''
   );
    CREATE INDEX IF NOT EXISTS idx_equipamentos_categoria_id ON equipamentos(categoria_id);
    CREATE TABLE IF NOT EXISTS distribuicao (
      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      vendedor TEXT NOT NULL,
      crm_id TEXT NOT NULL DEFAULT '',
      produto_id BIGINT REFERENCES produtos(id),
      faixa_id BIGINT REFERENCES faixas(id),
      modulos JSONB DEFAULT '[]'::jsonb,
      avulsos JSONB DEFAULT '[]'::jsonb,
      criado_em TIMESTAMPTZ DEFAULT NOW()
    );
    -- RLS policies: SELECT livre, escrita apenas autenticado
   ALTER TABLE usuarios ENABLE ROW LEVEL SECURITY;
   ALTER TABLE produtos ENABLE ROW LEVEL SECURITY;
   ALTER TABLE faixas ENABLE ROW LEVEL SECURITY;
    ALTER TABLE modulos ENABLE ROW LEVEL SECURITY;
   ALTER TABLE modulo_precos ENABLE ROW LEVEL SECURITY;
   ALTER TABLE leads ENABLE ROW LEVEL SECURITY;
   DROP POLICY IF EXISTS "usuarios_select" ON usuarios;
   DROP POLICY IF EXISTS "usuarios_update" ON usuarios;
   DROP POLICY IF EXISTS "usuarios_delete" ON usuarios;
   CREATE POLICY "usuarios_select" ON usuarios FOR SELECT
     USING (auth.uid() = auth_id);
   CREATE POLICY "usuarios_update" ON usuarios FOR UPDATE
     USING (auth.uid() = auth_id)
     WITH CHECK (auth.uid() = auth_id);
   CREATE POLICY "usuarios_delete" ON usuarios FOR DELETE
     USING (auth.role() = 'service_role');
   DROP POLICY IF EXISTS "produtos_select" ON produtos;
   DROP POLICY IF EXISTS "produtos_insert" ON produtos;
   DROP POLICY IF EXISTS "produtos_update" ON produtos;
   DROP POLICY IF EXISTS "produtos_delete" ON produtos;
   CREATE POLICY "produtos_select" ON produtos FOR SELECT USING (true);
   CREATE POLICY "produtos_insert" ON produtos FOR INSERT
     WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
   CREATE POLICY "produtos_update" ON produtos FOR UPDATE
     USING (auth.role() = 'authenticated' OR auth.role() = 'service_role')
     WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    CREATE POLICY "produtos_delete" ON produtos FOR DELETE
      USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    ALTER TABLE equip_categorias ENABLE ROW LEVEL SECURITY;
    ALTER TABLE equipamentos ENABLE ROW LEVEL SECURITY;
    DROP POLICY IF EXISTS "equip_categorias_select" ON equip_categorias;
    DROP POLICY IF EXISTS "equip_categorias_insert" ON equip_categorias;
    DROP POLICY IF EXISTS "equip_categorias_update" ON equip_categorias;
    DROP POLICY IF EXISTS "equip_categorias_delete" ON equip_categorias;
    CREATE POLICY "equip_categorias_select" ON equip_categorias FOR SELECT USING (true);
    CREATE POLICY "equip_categorias_insert" ON equip_categorias FOR INSERT
      WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    CREATE POLICY "equip_categorias_update" ON equip_categorias FOR UPDATE
      USING (auth.role() = 'authenticated' OR auth.role() = 'service_role')
      WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    CREATE POLICY "equip_categorias_delete" ON equip_categorias FOR DELETE
      USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    DROP POLICY IF EXISTS "equipamentos_select" ON equipamentos;
    DROP POLICY IF EXISTS "equipamentos_insert" ON equipamentos;
    DROP POLICY IF EXISTS "equipamentos_update" ON equipamentos;
    DROP POLICY IF EXISTS "equipamentos_delete" ON equipamentos;
    CREATE POLICY "equipamentos_select" ON equipamentos FOR SELECT USING (true);
    CREATE POLICY "equipamentos_insert" ON equipamentos FOR INSERT
      WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    CREATE POLICY "equipamentos_update" ON equipamentos FOR UPDATE
      USING (auth.role() = 'authenticated' OR auth.role() = 'service_role')
      WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    CREATE POLICY "equipamentos_delete" ON equipamentos FOR DELETE
      USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    ALTER TABLE distribuicao ENABLE ROW LEVEL SECURITY;
    DROP POLICY IF EXISTS "distribuicao_select" ON distribuicao;
    DROP POLICY IF EXISTS "distribuicao_insert" ON distribuicao;
    CREATE POLICY "distribuicao_select" ON distribuicao FOR SELECT USING (true);
    CREATE POLICY "distribuicao_insert" ON distribuicao FOR INSERT
      WITH CHECK (auth.role() = 'authenticated' OR auth.role() = 'service_role');
    GRANT SELECT, INSERT ON public.distribuicao TO anon;
    GRANT SELECT, INSERT ON public.distribuicao TO authenticated;
    -- (faixas, modulos, leads, equip_categorias, equipamentos seguem o mesmo padrão)

   -- Tabelas do Kanban de Tarefas
   CREATE TABLE IF NOT EXISTS tarefas (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     titulo TEXT NOT NULL,
     prioridade TEXT NOT NULL DEFAULT 'media' CHECK (prioridade IN ('alta', 'media', 'baixa')),
     status TEXT NOT NULL DEFAULT 'A Fazer',
     ordem INTEGER NOT NULL DEFAULT 0,
     nome_cliente TEXT DEFAULT '',
     codigo_cliente TEXT DEFAULT '',
     telefone TEXT DEFAULT '',
     data_vencimento DATE,
     descricao TEXT DEFAULT '',
     resumo TEXT DEFAULT '',
     anotacoes TEXT DEFAULT '',
     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS tarefas_historico (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     tarefa_id BIGINT NOT NULL REFERENCES tarefas(id) ON DELETE CASCADE,
     acao TEXT NOT NULL,
     de_status TEXT,
     para_status TEXT,
     user_id UUID,
     alterado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS colunas (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     nome TEXT NOT NULL,
     cor TEXT DEFAULT '#30d158',
     ordem INTEGER NOT NULL DEFAULT 0,
     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE
   );
   ALTER TABLE tarefas ENABLE ROW LEVEL SECURITY;
   ALTER TABLE tarefas_historico ENABLE ROW LEVEL SECURITY;
   ALTER TABLE colunas ENABLE ROW LEVEL SECURITY;
   DROP POLICY IF EXISTS "tarefas_select" ON tarefas;
   DROP POLICY IF EXISTS "tarefas_insert" ON tarefas;
   DROP POLICY IF EXISTS "tarefas_update" ON tarefas;
   DROP POLICY IF EXISTS "tarefas_delete" ON tarefas;
   CREATE POLICY "tarefas_select" ON tarefas FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "tarefas_insert" ON tarefas FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "tarefas_update" ON tarefas FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "tarefas_delete" ON tarefas FOR DELETE USING (auth.uid() = user_id);
   DROP POLICY IF EXISTS "tarefas_historico_select" ON tarefas_historico;
   DROP POLICY IF EXISTS "tarefas_historico_insert" ON tarefas_historico;
   CREATE POLICY "tarefas_historico_select" ON tarefas_historico FOR SELECT
     USING (auth.uid() = user_id);
   CREATE POLICY "tarefas_historico_insert" ON tarefas_historico FOR INSERT
     WITH CHECK (auth.uid() = user_id);
   DROP POLICY IF EXISTS "colunas_select" ON colunas;
   DROP POLICY IF EXISTS "colunas_insert" ON colunas;
   DROP POLICY IF EXISTS "colunas_update" ON colunas;
   DROP POLICY IF EXISTS "colunas_delete" ON colunas;
   CREATE POLICY "colunas_select" ON colunas FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "colunas_insert" ON colunas FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "colunas_update" ON colunas FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "colunas_delete" ON colunas FOR DELETE USING (auth.uid() = user_id);
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.tarefas TO authenticated;
   GRANT SELECT, INSERT ON public.tarefas_historico TO authenticated;
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.colunas TO authenticated;

   -- Tabelas do Painel Diario (Rotina Comercial)
   CREATE TABLE IF NOT EXISTS rotina_blocos (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
     nome TEXT NOT NULL,
     hora_inicio TEXT NOT NULL,
     hora_fim TEXT NOT NULL,
     descricao TEXT DEFAULT '',
     ordem INTEGER DEFAULT 0,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS rotina_checklist (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
     bloco TEXT NOT NULL,
     item TEXT NOT NULL,
     concluido BOOLEAN DEFAULT FALSE,
     data DATE NOT NULL,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   CREATE TABLE IF NOT EXISTS rotina_kpis (
     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
     data DATE NOT NULL,
     ligacoes_meta INTEGER DEFAULT 25,
     ligacoes_atual INTEGER DEFAULT 0,
     whatsapp_meta INTEGER DEFAULT 35,
     whatsapp_atual INTEGER DEFAULT 0,
     followups_meta INTEGER DEFAULT 15,
     followups_atual INTEGER DEFAULT 0,
     propostas_meta INTEGER DEFAULT 5,
     propostas_atual INTEGER DEFAULT 0,
     demonstracoes_meta INTEGER DEFAULT 2,
     demonstracoes_atual INTEGER DEFAULT 0,
     contatos_meta INTEGER DEFAULT 30,
     contatos_atual INTEGER DEFAULT 0,
     vendas_meta INTEGER DEFAULT 0,
     vendas_atual INTEGER DEFAULT 0,
     criado_em TIMESTAMPTZ DEFAULT NOW(),
     UNIQUE(user_id, data)
   );
   ALTER TABLE rotina_blocos ENABLE ROW LEVEL SECURITY;
   ALTER TABLE rotina_checklist ENABLE ROW LEVEL SECURITY;
   ALTER TABLE rotina_kpis ENABLE ROW LEVEL SECURITY;
   DROP POLICY IF EXISTS "rotina_blocos_select" ON rotina_blocos;
   DROP POLICY IF EXISTS "rotina_blocos_insert" ON rotina_blocos;
   DROP POLICY IF EXISTS "rotina_blocos_update" ON rotina_blocos;
   DROP POLICY IF EXISTS "rotina_blocos_delete" ON rotina_blocos;
   CREATE POLICY "rotina_blocos_select" ON rotina_blocos FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "rotina_blocos_insert" ON rotina_blocos FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "rotina_blocos_update" ON rotina_blocos FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "rotina_blocos_delete" ON rotina_blocos FOR DELETE USING (auth.uid() = user_id);
   DROP POLICY IF EXISTS "rotina_checklist_select" ON rotina_checklist;
   DROP POLICY IF EXISTS "rotina_checklist_insert" ON rotina_checklist;
   DROP POLICY IF EXISTS "rotina_checklist_update" ON rotina_checklist;
   DROP POLICY IF EXISTS "rotina_checklist_delete" ON rotina_checklist;
   CREATE POLICY "rotina_checklist_select" ON rotina_checklist FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "rotina_checklist_insert" ON rotina_checklist FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "rotina_checklist_update" ON rotina_checklist FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "rotina_checklist_delete" ON rotina_checklist FOR DELETE USING (auth.uid() = user_id);
   DROP POLICY IF EXISTS "rotina_kpis_select" ON rotina_kpis;
   DROP POLICY IF EXISTS "rotina_kpis_insert" ON rotina_kpis;
   DROP POLICY IF EXISTS "rotina_kpis_update" ON rotina_kpis;
   CREATE POLICY "rotina_kpis_select" ON rotina_kpis FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "rotina_kpis_insert" ON rotina_kpis FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "rotina_kpis_update" ON rotina_kpis FOR UPDATE USING (auth.uid() = user_id);
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.rotina_blocos TO authenticated;
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.rotina_checklist TO authenticated;
   GRANT SELECT, INSERT, UPDATE ON public.rotina_kpis TO authenticated;
   ```
5. Enable **Email/Password** provider in **Authentication > Providers > Email**.
6. Create an admin user via **Authentication > Users > Add User**:
   - **Email**: `{nome}@songbird.app` (ex: `felipe@songbird.app`)
   - **Password**: sua senha
   - **Auto-confirm user**: ✅
   - **User Metadata**: `{"nome": "felipe", "permissao": "admin"}`
7. Link the user to the `usuarios` table:
   ```sql
   UPDATE usuarios SET auth_id = '<UUID do usuario criado>' WHERE id = 1;
   ```

## Commands

| Command | Purpose |
|---------|---------|
| `node migrate-supabase.js` | Migrate from legacy SQLite `dados.db` to Supabase. Requires `npm install @supabase/supabase-js sql.js`. Uses `SUPABASE_SERVICE_ROLE_KEY` do `.env`. |

## Versioning

- Version number is stored in `public/js/supabase-init.js` (`const SONGBIRD_VERSION`) and `package.json`.
- On each deploy (after significant changes or grouped commits):
  1. Increment version in both files (semver: major.minor.patch)
  2. Add entry to `CHANGELOG.md` describing what changed
  3. The version is displayed in the app sidebar footer and terminal logo

## Authentication

- Uses **Supabase Auth** (Email/Password) — not SHA-256.
- Login é feito via `supabase.auth.signInWithPassword()` com email no formato `{nome}@songbird.app`.
- Sessão gerenciada automaticamente pelo Supabase JS client (localStorage).
- O `.env` deve conter `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` (não a anon key).

## Deployment

- **GitHub Pages** via `gh-pages` branch.
- `main` branch has all source files.
- `gh-pages` branch has `index.html` (from `public/`) at root with real Supabase credentials.
- The frontend is static HTML/JS — no server, no env injection. Credentials are hardcoded in the file.

## Important gotchas

- `public/js/supabase-init.js:3-4` has hardcoded Supabase credentials that must be replaced with real ones before the app works.
- Toda **nova tabela** criada manualmente no Supabase precisa de `GRANT SELECT, INSERT, UPDATE, DELETE ON public.<tabela> TO anon;` (e/ou `authenticated`) antes que a REST API funcione. As tabelas do setup inicial já incluem isso, mas tabelas adicionadas depois não — o Postgres não concede permissões automaticamente.
- `DESIGN.md` is for the OpenCode marketing site design system, unrelated to this project.
- `migrate-supabase.js` is a one-shot script for migrating legacy SQLite data.
- Password is SHA-256, **not bcrypt**. Both client-side login (and server-side migration) use it.
- Gerenciamento de usuários (criar/editar/deletar) é feito exclusivamente pelo Dashboard do Supabase (Authentication > Users) — o frontend não tem mais essa aba.
- O login agora usa **Supabase Auth** (`supabase.auth.signInWithPassword`). O email é construído como `{nome}@songbird.app`.
- A senha é gerenciada pelo Supabase Auth (bcrypt), não mais pela tabela `usuarios`.
- Arquivos `*.sql` são locais e **nunca** devem ser commitados. Estão no `.gitignore`.
- O schema SQL está embutido neste arquivo na seção Supabase setup — copie de lá para o SQL Editor.

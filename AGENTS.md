...
4. Run the SQL below in the SQL Editor (creates schema + RLS policies):
   ...
   -- Tabelas da Agenda (Gestão de Tempo)
   ...
   CREATE TABLE IF NOT EXISTS agenda_tarefas (
     ...
     titulo TEXT NOT NULL,
     descricao TEXT DEFAULT '',
     projeto_id BIGINT REFERENCES agenda_projetos(id) ON DELETE SET NULL,
     prioridade INTEGER DEFAULT 3 CHECK (prioridade IN (1, 2, 3, 4)),
     data_vencimento DATE,
     recorrencia TEXT CHECK (recorrencia IN ('diaria', 'semanal', 'mensal')),
     recorrencia_dia INTEGER,
     tarefa_pai_id BIGINT REFERENCES agenda_tarefas(id) ON DELETE CASCADE,
     concluido BOOLEAN DEFAULT FALSE,
     concluido_em TIMESTAMPTZ,
     ordem INTEGER DEFAULT 0,
     criado_em TIMESTAMPTZ DEFAULT NOW()
   );
   ALTER TABLE agenda_projetos ENABLE ROW LEVEL SECURITY;
   ALTER TABLE agenda_tarefas ENABLE ROW LEVEL SECURITY;
   DROP POLICY IF EXISTS "agenda_projetos_select" ON agenda_projetos;
   DROP POLICY IF EXISTS "agenda_projetos_insert" ON agenda_projetos;
   DROP POLICY IF EXISTS "agenda_projetos_update" ON agenda_projetos;
   DROP POLICY IF EXISTS "agenda_projetos_delete" ON agenda_projetos;
   CREATE POLICY "agenda_projetos_select" ON agenda_projetos FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "agenda_projetos_insert" ON agenda_projetos FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "agenda_projetos_update" ON agenda_projetos FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "agenda_projetos_delete" ON agenda_projetos FOR DELETE USING (auth.uid() = user_id);
   DROP POLICY IF EXISTS "agenda_tarefas_select" ON agenda_tarefas;
   DROP POLICY IF EXISTS "agenda_tarefas_insert" ON agenda_tarefas;
   DROP POLICY IF EXISTS "agenda_tarefas_update" ON agenda_tarefas;
   DROP POLICY IF EXISTS "agenda_tarefas_delete" ON agenda_tarefas;
   CREATE POLICY "agenda_tarefas_select" ON agenda_tarefas FOR SELECT USING (auth.uid() = user_id);
   CREATE POLICY "agenda_tarefas_insert" ON agenda_tarefas FOR INSERT WITH CHECK (auth.uid() = user_id);
   CREATE POLICY "agenda_tarefas_update" ON agenda_tarefas FOR UPDATE USING (auth.uid() = user_id);
   CREATE POLICY "agenda_tarefas_delete" ON agenda_tarefas FOR DELETE USING (auth.uid() = user_id);
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.agenda_projetos TO authenticated;
   GRANT SELECT, INSERT, UPDATE, DELETE ON public.agenda_tarefas TO authenticated;
   ```
